...
public class BeehiveDAOImpl implements BeehiveDAO {
	...
	public void addBeehive(Beehive bh) {
		...
		try {
			...
			/*  
			 * Persist: save to db and has a void return 
			 * Save: save to db and returns a serializable identifier value (id).
			 */
			sess.persist(bh);
	...
	try {
		...
		/*
		 * Update vs. Merge
		 * Hibernate does not allow 2 objects
		 * with the same id in the same session.
		 * 
		 * 'Update' on an object in the session with the
		 * same id will throw an exception, because it 
		 * doesn't know which object should be changed.
		 * 
